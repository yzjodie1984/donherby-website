...
const getImages = (query) => new Promise(resolve => {
  https.get('https://api.pexels.com/v1/search?query=' + encodeURIComponent(query) + '&per_page=5', {
    ...
    res.on('end', () => {
      try {
        const url = JSON.parse(str).photos[0].src.large;
        resolve(query + ': ' + url);
...
async function main() {
  console.log(await getImages('cargo ship container'));
  console.log(await getImages('cargo airplane'));
  console.log(await getImages('freight train'));
  console.log(await getImages('shipping containers logistics'));
  console.log(await getImages('warehouse interior boxes'));
  console.log(await getImages('customs shipping documents'));
}
main();
