...
async function processFiles() {
  ...
  for (const url of originalUrls) {
    ...
    const publicPath = `/images/img_${hash}.jpg`;
    hashMap[publicPath] = cleanUrl;
  }
  ...
  for (const file of files) {
    ...
    let updated = false;
    for (const [hashPath, originalUrl] of Object.entries(hashMap)) {
      if (content.includes(hashPath)) {
        content = content.replace(new RegExp(hashPath.replace(/\//g, '\\/'), 'g'), originalUrl);
        updated = true;
      }
    }
